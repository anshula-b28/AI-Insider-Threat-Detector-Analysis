...
# VIEW 5: REPORTS (ROUTING AND FORMATTING FIXED)
elif selected_view == "📈 Reports":
    st.title("🛡️ Security Audit Compliance Reports")
    st.markdown("---")
    st.info("💡 **Compliance Report Generated Successfully**")
    st.write(f"Total documented insider risk events captured today: **{len(high_risk_employees)}** active anomalies.")
    
    # 1. Display the clean visual table frame right on the screen
    st.dataframe(high_risk_employees, use_container_width=True, hide_index=True)
    
    # 2. Compile background cell-width space padding transformations for WPS Office/Excel compatibility
    ...
    padded_df['Severity']            = padded_df['Severity'].astype(str).apply(lambda x: f"{x:<12}")
    
    csv_data_content = padded_df.to_csv(index=False)
    
    # 3. Interactive padded file data download trigger link
    st.download_button(
        label="📥 Download Comprehensive Incident Audit CSV",
