...
# VIEW 5: REPORTS
    ...
    # 1. Create a clean copy of your DataFrame to apply formatting
padded_df = df.copy()

# 2. Add trailing space padding to push column boundaries outward
...
padded_df['Incident Id']       = padded_df['Incident Id'].astype(str).apply(lambda x: f"{x:<28}")
padded_df['Severity']          = padded_df['Severity'].astype(str).apply(lambda x: f"{x:<12}")

# 3. Convert the padded dataframe into a standard CSV data stream
csv_data_content = padded_df.to_csv(index=False)

# 4. Render the download trigger component
st.download_button(
